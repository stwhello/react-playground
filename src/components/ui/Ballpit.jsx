...
class ThreeApp {
  canvas; camera; cameraFov; scene; renderer;
  size = { width:0, height:0, wWidth:0, wHeight:0, ratio:0, pixelRatio:0 };
  onBeforeRender=()=>{}; onAfterRender=()=>{}; onAfterResize=()=>{};
  #cfg; #visible=false; #running=false; #resizeTimeout; #resizeObserver; #intersectionObserver; #rafId; #clock=new e(); #time={elapsed:0,delta:0};
  ...
  constructor(cfg) {
    ...
    this.renderer = new s({ canvas: this.canvas, powerPreference: 'high-performance', antialias: true, alpha: true });
  ...
  resize() {
    ...
    this.renderer.setSize(w, h); this.renderer.setPixelRatio(dpr); this.size.pixelRatio = dpr;
  ...
  #startLoop() {
    ...
    this.#running = true; this.#clock.reset();
    const loop = () => {
  ...
  #stopLoop() {
    if (this.#running) { cancelAnimationFrame(this.#rafId); this.#running = false; }
  }

  clear() {
    this.scene.traverse(el => {
      if (el.isMesh) {
        Object.keys(el.material).forEach(k => { const val = el.material[k]; if (val && typeof val.dispose === 'function') val.dispose(); });
        el.material.dispose(); el.geometry.dispose();
      }
    });
    this.scene.clear();
  }
  ...
  dispose() {
    ...
    this.#stopLoop(); this.#clock.dispose(); this.clear();
    this.renderer.dispose(); this.renderer.forceContextLoss();
  }
}

const pointerMap = new Map(); let globalListening = false;
const globalPos = new r();

function createPointer(cfg) {
  const state = { position: new r(), nPosition: new r(), hover: false, touching: false, onEnter(){}, onMove(){}, onClick(){}, onLeave(){}, ...cfg };
...
function updatePos(x, y, st, rect) { st.position.set(x - rect.left, y - rect.top); st.nPosition.set((st.position.x / rect.width) * 2 - 1, (-st.position.y / rect.height) * 2 + 1); }
function inRect(rect) { return globalPos.x >= rect.left && globalPos.x <= rect.left + rect.width && globalPos.y >= rect.top && globalPos.y <= rect.top + rect.height; }
function onPointerMove(e) { globalPos.set(e.clientX, e.clientY); for (const [el, st] of pointerMap) { const rect = el.getBoundingClientRect(); if (inRect(rect)) { updatePos(e.clientX, e.clientY, st, rect); if (!st.hover) { st.hover = true; st.onEnter(st); } st.onMove(st); } else if (st.hover && !st.touching) { st.hover = false; st.onLeave(st); } } }
function onPointerLeave() { for (const st of pointerMap.values()) { if (st.hover) { st.hover = false; st.onLeave(st); } } }
function onPointerClick(e) { globalPos.set(e.clientX, e.clientY); for (const [el, st] of pointerMap) { const rect = el.getBoundingClientRect(); updatePos(e.clientX, e.clientY, st, rect); if (inRect(rect)) st.onClick(st); } }
function onTouchStart(e) { if (!e.touches.length) return; e.preventDefault(); globalPos.set(e.touches[0].clientX, e.touches[0].clientY); for (const [el, st] of pointerMap) { const rect = el.getBoundingClientRect(); if (inRect(rect)) { st.touching = true; updatePos(e.touches[0].clientX, e.touches[0].clientY, st, rect); if (!st.hover) { st.hover = true; st.onEnter(st); } st.onMove(st); } } }
function onTouchMove(e) { if (!e.touches.length) return; e.preventDefault(); globalPos.set(e.touches[0].clientX, e.touches[0].clientY); for (const [el, st] of pointerMap) { const rect = el.getBoundingClientRect(); updatePos(e.touches[0].clientX, e.touches[0].clientY, st, rect); if (inRect(rect)) { if (!st.hover) { st.hover = true; st.touching = true; st.onEnter(st); } st.onMove(st); } else if (st.hover && st.touching) st.onMove(st); } }
function onTouchEnd() { for (const st of pointerMap.values()) { if (st.touching) { st.touching = false; if (st.hover) { st.hover = false; st.onLeave(st); } } } }

const { randFloat: k, randFloatSpread: E } = o;
const v1=new a(),v2=new a(),v3=new a(),v4=new a(),v5=new a(),v6=new a(),v7=new a(),v8=new a(),v9=new a(),v10=new a();
...
class Physics {
  ...
  #init() {
    ...
    for (let i = 1; i < c.count; i++) { const b = 3*i; p[b]=E(2*c.maxX); p[b+1]=E(2*c.maxY); p[b+2]=E(2*c.maxZ); }
  }
  setSizes() {
  ...
  update(e) {
    ...
    let start = 0;
    if (cfg.controlSphere0) { start=1; v1.fromArray(pos,0); v1.lerp(ctr,0.1).toArray(pos,0); v4.set(0,0,0).toArray(vel,0); }
    for (let i = start; i < cfg.count; i++) { const b=3*i; v2.fromArray(pos,b); v5.fromArray(vel,b); v5.y-=e.delta*cfg.gravity*sz[i]; v5.multiplyScalar(cfg.friction); v5.clampLength(0,cfg.maxVelocity); v2.add(v5); v2.toArray(pos,b); v5.toArray(vel,b); }
    for (let i = start; i < cfg.count; i++) {
      const b=3*i; v2.fromArray(pos,b); v5.fromArray(vel,b); const ri=sz[i];
      for (let j=i+1; j<cfg.count; j++) { const ob=3*j; v3.fromArray(pos,ob); v6.fromArray(vel,ob); const rj=sz[j]; v7.copy(v3).sub(v2); const dist=v7.length(),sr=ri+rj; if(dist<sr){const ov=sr-dist; v8.copy(v7).normalize().multiplyScalar(0.5*ov); v9.copy(v8).multiplyScalar(Math.max(v5.length(),1)); v10.copy(v8).multiplyScalar(Math.max(v6.length(),1)); v2.sub(v8); v5.sub(v9); v2.toArray(pos,b); v5.toArray(vel,b); v3.add(v8); v6.add(v10); v3.toArray(pos,ob); v6.toArray(vel,ob);} }
      if (cfg.controlSphere0) { v7.copy(v1).sub(v2); const dist=v7.length(),sr0=ri+sz[0]; if(dist<sr0){const diff=sr0-dist; v8.copy(v7.normalize()).multiplyScalar(diff); v9.copy(v8).multiplyScalar(Math.max(v5.length(),2)); v2.sub(v8); v5.sub(v9);} }
      if (Math.abs(v2.x)+ri>cfg.maxX){v2.x=Math.sign(v2.x)*(cfg.maxX-ri);v5.x=-v5.x*cfg.wallBounce;}
      if (cfg.gravity===0){if(Math.abs(v2.y)+ri>cfg.maxY){v2.y=Math.sign(v2.y)*(cfg.maxY-ri);v5.y=-v5.y*cfg.wallBounce;}}
      else if(v2.y-ri<-cfg.maxY){v2.y=-cfg.maxY+ri;v5.y=-v5.y*cfg.wallBounce;}
      const mb=Math.max(cfg.maxZ,cfg.maxSize); if(Math.abs(v2.z)+ri>mb){v2.z=Math.sign(v2.z)*(cfg.maxZ-ri);v5.z=-v5.z*cfg.wallBounce;}
...
class BallpitMesh extends d {
  ...
  setColors(colors) {
    ...
    const getAt = (ratio, out = new l()) => {
      ...
      out.r = s.r + alpha*(e.r-s.r); out.g = s.g + alpha*(e.g-s.g); out.b = s.b + alpha*(e.b-s.b);
      return out;
    };
